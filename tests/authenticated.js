...
test('get account', function(done) {
  ...
  nock(EXCHANGE_API_URL).get('/accounts/test-id').reply(200, expectedResponse);
...
test('get accounts', function(done) {
  ...
  nock(EXCHANGE_API_URL).get('/accounts').reply(200, expectedResponse);
...
test('get product orderbook', function(done) {
  ...
  authClient.getProductOrderBook({ level: 3 }, 'BTC-USD', function(
    err,
    resp,
    data
  ) {
...
test('cancel all orders', function(done) {
  ...
  const totalExpectedDeleted = cancelledOrdersOne.concat(cancelledOrdersTwo);

  // first list of Id's that just got cancelled
  nock(EXCHANGE_API_URL).delete('/orders').reply(200, cancelledOrdersOne);

  // second list of Id's that just got cancelled
  nock(EXCHANGE_API_URL).delete('/orders').reply(200, cancelledOrdersTwo);

  // return empty array signalling the completion of all
  // orders having been cancelled
  nock(EXCHANGE_API_URL).delete('/orders').reply(200, []);
...
test('get orders', function(done) {
  ...
  nock(EXCHANGE_API_URL).get('/orders').reply(200, expectedResponse);
...
test('get fills', function(done) {
  ...
  nock(EXCHANGE_API_URL).get('/fills').reply(200, expectedResponse);
...
test('get fundings', function(done) {
  ...
  nock(EXCHANGE_API_URL).get('/funding').reply(200, expectedResponse);
...
test('close position', function(done) {
  ...
  nock(EXCHANGE_API_URL).post('/position/close', params).reply(200, {});
...
test('deposit', function(done) {
  ...
  nock(EXCHANGE_API_URL).post('/transfers', expectedTransfer).reply(200, {});
